    ...
    <div className='group grid grid-cols-4 gap-4 md:gap-8 mt-8 max-w-3xl mx-auto' >
     {
      dummyTrailers.map((trialer)=>(
        ...
        <div key={trialer.image} className='relative group-hover:not-hover:opacity-50 hover:-translate-y-1 duration-300 transition max-md:h-60 md:max-h-60 cursor-pointer' onClick={()=>{setCurrentTrailer(trialer)}}>
          <img src={trialer.image} alt="trialer" className='rounded-lg w-full h-full object-cover brightness-75' />
